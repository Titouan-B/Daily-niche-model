...
print('Script started, Logistic G', flush=True)
...
incompatibility_lim = 2       # maximum genetic distance allowing breeding
...
def emergences_and_schedule(pop,incompatibility_lim,ve1,e1,K,mu,current_day):   
    ...
    for i in range(len(Mated_females)): 
        ...
        b_logi = (b*logistic(dist,incompatibility_lim,k=20))

        offsp = np.random.poisson(b_logi)   #Number of offsprings
...
def ReplicatesSimulation(queue,qn,simulation_days, Output):     # Main function to loop through replications/parameters
    ...
    for k in range(simulation_days): # for desired duration of replicates
        ...
        emergences_today = emergences_and_schedule(population,incompatibility_lim,ve1,e1,K,mu,k)                        # retrieves emergence list and emergence schedule of the day
...
Test_value = "dlt_c"         # Put "e" if you want to test the effect of emergence values
print(Test_value) 
