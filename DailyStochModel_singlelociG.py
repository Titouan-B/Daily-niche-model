...
print('Script started, single G', flush=True)
...
def g_transfer(femelle, male): 
    ...
    gene = stats.truncnorm.rvs(lower - (femelle+male/2)/mu_g,(upper-(femelle+male)/2)/mu_g, (femelle+male)/2, mu_g)           
    return gene
...
def emergences_and_schedule(pop,incompatibility_lim,ve1,e1,K,mu,mu_G,current_day):   
    ...
    for i in range(len(Mated_females)): 
        ...
        if abs(Mated_females[i].gene - Mated_females[i].gene_male[r_ind]) <= incompatibility_lim:
            ...
            for j in range(offsp):  
                sex = r.sample(["M","F"],1)[0]
                emergence_list.append(individuals(sex,         # creation of new emerging individuals
...
def ReplicatesSimulation(queue,qn,simulation_days, Output):     # Main function to loop through replications/parameters
    ...
    for k in range(simulation_days): # for desired duration of replicates
        ...
        emergences_today = emergences_and_schedule(population,incompatibility_lim,ve1,e1,K,mu,mu_g,k)                        # retrieves emergence list and emergence schedule of the day
        ...
        generation_time.append(total_lifespan)
    ...
    Output.put([repli, 
                ...
                [list_e(population)],
                [ha_list],[female_ha_list],
                [sex_ratio_sp1(population)],
                [generation_time],
                list_gene(population),list_gene_of_females(population), 
                incompatibility_lim,
                n])
...
param = 10                # Number of parameters tested
...
for i in range(val_n):      # Generate data for each parameter
    ...
    for rep in range(i*replications,(i+1)*replications):
        ...
        if len(ha_list[rep][0])==simulation_days:
            ...
            dic = {}
            for j,k in enumerate(ha_list[rep][0][-1]):
                dic[k] = gene[rep][j]
            # print('gene')
            # print(gene[rep])
            # print("dic")
            ...
            if size_sup*size_inf !=0:
                ...
                for sup in range(size_sup):
                    for sup2 in range(size_sup):
                        dfsup += abs(ha_sup[sup] - ha_sup[sup2])
                ...
                for inf in range(size_inf):
                    for inf2 in range(size_inf):
                        dfinf += abs(ha_inf[inf] - ha_inf[inf2])
